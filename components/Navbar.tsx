    ...
    return (
        <nav className='mx-auto max-w-[1440px] padding-container relative z-30 py-5 p-5 flex justify-between items-center'>
            ...
            <ul className='h-full gap-12 lg:flex hidden'>
                {NAV_LINKS.map(link => (
                    <Link href={link.href} key={link.key} className='font-light text-gray-500 flex-center cursor-pointer pb-1.5 transition-all hover:font-bold  hover:text-neutral-800  hover:underline hover:decoration-solid' style={{ textDecorationColor: 'orange' }}>
                        {link.label}
                    </Link>
                ))}
            </ul>
